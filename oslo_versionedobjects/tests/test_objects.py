...
class MyObj(base.VersionedObject, base.VersionedObjectDictCompat):
    ...
    def obj_make_compatible(self, primitive, target_version):
        ...
        if target_version == '1.1' and 'bar' in primitive:
            primitive['bar'] = 'old{}'.format(primitive['bar'])
...
class TestRegistry(test.TestCase):
    def test_obj_tracking(self):
        ...
        # newest object.
        expected = {
            'fake1': [Fake1TestObj4, Fake1TestObj5, Fake1TestObj2],
    ...
    def test_field_checking(self):
        ...
        self.assertRaises(
            exception.ObjectFieldInvalid, create_class, fields.DateTime
        )
    ...
    def test_subclassability(self):
        ...
        self.assertIn(
            'AVersionedObject1', base.VersionedObjectRegistry.obj_classes()
        )
        self.assertIn(
...
class TestDoSubobjectBackport(test.TestCase):
    ...
    def test_do_subobject_backport_without_manifest(self):
        ...
        with mock.patch.object(child, compat_func) as mock_compat:
            base._do_subobject_backport(
                version, parent, 'child', parent_primitive
            )
            mock_compat.assert_called_once_with(
                primitive, version, version_manifest=None
            )
    ...
    def test_do_subobject_backport_with_manifest(self):
        ...
        with mock.patch.object(child, compat_func) as mock_compat:
            ...
            mock_compat.assert_called_once_with(
                primitive, version, version_manifest=manifest
            )
    ...
    def test_do_subobject_backport_with_manifest_old_parent(self):
        ...
        parent = self.ParentObj(child=child)
        manifest = {'ChildObj': '1.0'}
        parent_primitive = parent.obj_to_primitive(
            target_version='1.1', version_manifest=manifest
        )
    ...
    def test_do_subobject_backport_list_object(self):
        ...
        with mock.patch.object(child, compat_func) as mock_compat:
            base._do_subobject_backport(
                version, parent, 'objects', parent_primitive
            )
            mock_compat.assert_called_once_with(
                primitive, version, version_manifest=None
            )
    ...
    def test_do_subobject_backport_list_object_with_manifest(self):
        ...
        parent_primitive = parent.obj_to_primitive(
            target_version='1.0', version_manifest=manifest
        )
    ...
    def test_to_primitive_calls_make_compatible_manifest(self):
        ...
        with mock.patch.object(obj, 'obj_make_compatible_from_manifest') as m:
            obj.obj_to_primitive(
                target_version='1.0', version_manifest=mock.sentinel.manifest
            )
...
class TestFixture(_BaseTestCase):
    def test_fake_indirection_takes_serializer(self):
        ...
        ser.serialize_entity.assert_called_once_with(
            mock.sentinel.context, mock.sentinel.objinst
        )
        ser.deserialize_entity.assert_called_once_with(
            mock.sentinel.context, mock.sentinel.serial
        )
    ...
    def test_indirection_action(self):
        ...
        with mock.patch.object(
            base.VersionedObject.indirection_api, 'object_action'
        ) as mock_action:
            mock_action.return_value = ({}, 'foo')
            obj.marco()
            mock_action.assert_called_once_with(
                self.context, obj, 'marco', (), {}
            )
    ...
    def test_indirection_class_action(self, mock_otgv):
        ...
        self.useFixture(fixture.IndirectionFixture())
        with mock.patch.object(
            base.VersionedObject.indirection_api,
            'object_class_action_versions',
        ) as mock_caction:
            mock_caction.return_value = 'foo'
            MyObj.query(self.context)
            mock_caction.assert_called_once_with(
                self.context, 'MyObj', 'query', mock.sentinel.versions, (), {}
            )
    ...
    def test_get_dependency_tree(self):
        ...
        # to change. Otherwise, leave it alone.
        self.assertEqual(
            {'MyOwnedObject': '1.0'}, tree['TestSubclassedObject']
        )
    ...
    def test_test_compatibility(self):
        fake_classes = {
            mock.sentinel.class_one: [
                mock.sentinel.impl_one_one,
                mock.sentinel.impl_one_two,
            ],
            mock.sentinel.class_two: [
                mock.sentinel.impl_two_one,
    ...
    def test_test_relationships_in_order_good(self):
        ...
        fake.obj_relationships = {'foo': [('1.2', '1.0'), ('1.3', '1.2')]}
    ...
    def _test_test_relationships_in_order_bad(self, fake_rels):
        ...
        checker = fixture.ObjectVersionChecker()
        self.assertRaises(
            AssertionError, checker._test_relationships_in_order, fake
        )
...
class _TestObject:
    ...
    def test_hydration_additional_unexpected_stuff(self):
        primitive = {
            ...
            'versioned_object.data': {'foo': 1, 'unexpected_thing': 'foobar'},
        }
        obj = MyObj.obj_from_primitive(primitive)
    ...
    def test_loaded_in_primitive(self):
        ...
        expected = {
            ...
            'versioned_object.data': {'foo': 1, 'bar': 'loaded!'},
        }
        self.assertEqual(obj.obj_to_primitive(), expected)
    ...
    def test_obj_reset_changes_recursive(self):
        obj = MyObj(
            rel_object=MyOwnedObject(baz=123),
    ...
    def test_obj_context(self):
        ...
        self.assertRaises(
            AttributeError, setattr, obj, 'obj_context', new_context
        )
    ...
    def test_obj_read_only(self):
        ...
        obj.readonly = 1
        self.assertRaises(
            exception.ReadOnlyFieldError, setattr, obj, 'readonly', 2
        )
    ...
    def test_obj_make_obj_compatible_with_relationships(self):
        ...
        with mock.patch.object(subobj, 'obj_make_compatible') as mock_compat:
            obj._obj_make_obj_compatible(
                copy.copy(primitive), '1.8', 'rel_object'
            )
            self.assertFalse(mock_compat.called)

        with mock.patch.object(subobj, 'obj_make_compatible') as mock_compat:
            obj._obj_make_obj_compatible(
                copy.copy(primitive), '1.7', 'rel_object'
            )
            mock_compat.assert_called_once_with(
        ...
        with mock.patch.object(subobj, 'obj_make_compatible') as mock_compat:
            obj._obj_make_obj_compatible(
                copy.copy(primitive), '1.6', 'rel_object'
            )
            mock_compat.assert_called_once_with(
        ...
        with mock.patch.object(subobj, 'obj_make_compatible') as mock_compat:
            obj._obj_make_obj_compatible(
                copy.copy(primitive), '1.5', 'rel_object'
            )
            mock_compat.assert_called_once_with(
                primitive['rel_object']['versioned_object.data'], '1.1'
            )
    ...
    def test_obj_make_compatible_hits_sub_objects_with_rels(self):
        ...
        with mock.patch.object(obj, '_obj_make_obj_compatible') as mock_compat:
            ...
            mock_compat.assert_called_once_with(
                {'rel_object': 'foo'}, '1.10', 'rel_object'
            )
    ...
    def test_obj_make_compatible_complains_about_missing_rel_rules(self):
        ...
        obj.obj_relationships = {}
        self.assertRaises(
            exception.ObjectActionError, obj.obj_make_compatible, {}, '1.0'
        )
    ...
    def test_obj_make_compatible_on_list_base(self):
        ...
        with mock.patch.object(childobj, compat_func) as mock_compat:
            ...
            mock_compat.assert_called_once_with(
                {'foo': 1}, '1.0', version_manifest=None
            )
...
class TestObject(_LocalTest, _TestObject):
    ...
    def test_set_defaults_no_default(self):
        obj = MyObj()
        self.assertRaises(
            exception.ObjectActionError, obj.obj_set_defaults, 'bar'
        )

    def test_set_all_defaults(self):
        obj = MyObj()
        obj.obj_set_defaults()
        self.assertEqual({'mutable_default', 'foo'}, obj.obj_what_changed())
        self.assertEqual(1, obj.foo)
...
class TestRemoteObject(_RemoteTest, _TestObject):
    ...
    def test_major_version_mismatch(self, mock_otgv):
        ...
        self.assertRaises(
            exception.IncompatibleObjectVersion, MyObj2.query, self.context
        )
...
class TestObjectListBase(test.TestCase):
    def test_list_like_operations(self):
        ...
        objlist = Foo(
            context='foo', objects=[MyElement(1), MyElement(2), MyElement(3)]
        )
    ...
    def _test_object_list_version_mappings(self, list_obj_class):
        ...
        for item_class in obj_classes:
            ...
            self.assertIn(
                ...
                f'Version mapping is incomplete for {list_obj_class.__name__}',
            )
...
class TestObjectSerializer(_BaseTestCase):
    ...
    def _test_deserialize_entity_newer(
        self, obj_version, backported_to, mock_iapi, my_version='1.6'
    ):
        ser = base.VersionedObjectSerializer()
        ...
        if backported_to is None:
            ...
            mock_iapi.object_backport_versions.assert_called_with(
                self.context,
                primitive,
                {'MyTestObj': my_version, 'MyOwnedObject': '1.0'},
            )
    ...
    def _test_nested_backport(self, old):
        ...
        with mock.patch.object(base.VersionedObject, 'indirection_api') as a:
            ...
            a.object_backport_versions.assert_called_once_with(
                self.context,
                prim,
                {'Parent': '1.1', 'MyObj': '1.6', 'MyOwnedObject': '1.0'},
            )
            if old:
    ...
    def test_serializer_subclass_namespace_mismatch(self):
        ...
        obj = MyObj(foo=123)
        obj2 = myser.deserialize_entity(
            None, voser.serialize_entity(None, obj)
        )
...
class TestSchemaGeneration(test.TestCase):
    ...
    def test_to_json_schema(self):
        ...
        jsonschema.validate(
            self.FakeObject(a_boolean=True).obj_to_primitive(),
    ...
    def test_to_json_schema_complex_object(self):
        ...
        fake_obj = self.FakeComplexObject(
            a_dict={'key1': ['foo', 'bar'], 'key2': ['bar', 'baz']},
...
class TestNamespaceCompatibility(test.TestCase):
    ...
    def test_obj_primitive_key(self):
        self.assertEqual(
            'foo.data', self.test_class._obj_primitive_key('data')
        )
    ...
    def test_obj_primitive_field(self):
        ...
        self.assertEqual(
            mock.sentinel.data,
            self.test_class._obj_primitive_field(primitive, 'data'),
        )
    ...
    def test_obj_primitive_field_namespace_wrong(self):
        ...
        self.assertRaises(
            exception.UnsupportedObjectError,
            self.test_class.obj_from_primitive,
            primitive,
        )
